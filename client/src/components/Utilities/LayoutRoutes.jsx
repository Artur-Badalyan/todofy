...
import ButtonsSort from "components/TasksSection/ButtonsSort";
import TaskItem from "components/TasksSection/TaskItem/TaskItem";
...
const LayoutRoutes = ({ title, tasks }) => {
  ...
  return (<section>
    ...
    <ul className={`tasksList mt-4 grid gap-2 sm:gap-4 xl:gap-6 ${isListInView1
          ? "grid-cols-1"
          : "2xl:grid-cols-4 xl:grid-cols-3 lg:grid-cols-4 md:grid-cols-3 grid-cols-2 items-end"}`}>
      {sortedTasks.map((task) => (<TaskItem key={task.id} isListInView1={isListInView1} task={task}/>))}
      <li>
        <button onClick={openModalHandler} className={`border-2 border-slate-300
